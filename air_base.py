...
if len(sys.argv) > 3:
  ...
  print "Warn: No amount was provided, so air bases were set to a value of 10/10"
  amount='10.000'
text_file = open(input_file, "r")
whole_thing = text_file.read()
text_file.close()
text_file = open(input_file, "r")
#pattern=re.compile('air_base=.+?}', re.MULTILINE|re.DOTALL)
...
for idx, line in enumerate(text_file):
  ...
  if not reached_end_of_provinces:
    ...
    if match_nation_flag:
      if re.search('air_base=',line):
        delay_write=True
      if delay_write:
        if re.search('}',line):
          delay_write=False
          line='air_base={' + amount + ' ' + amount + '}\n'
  if not delay_write:
    outFile.write(line)
text_file.close()
outFile.close()
