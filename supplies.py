...
#      if entered_subelement:
      supplies_match = re.search('supplies=',line)
      if supplies_match:
        line='supplies=' + str(float(line[supplies_match.end():re.search('[0-9].[0-9]*',line[supplies_match.end():]).end()+supplies_match.end()]) * multiplier) + '\n'
  if not delay_write:
    outFile.write(line)
text_file.close()
outFile.close()
